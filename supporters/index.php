<?php include('../header.php'); ?>


<div id="dynamic">
  <div class="supporters">
  	<div id="latest_news">
  		<ul id="latest_header" class="headlineLargeGray">
	  		<li class="supporters_header"><span>LATEST NEWS</span></li>
	  		<li id="follow"><span>Follow</span><a href="" id="facebook_follow"><img src="../images/facebook_follow.png"></a><a href=""><img id="google_follow" src="../images/google_follow.png"></a></li>
  		</ul>
  		<ul id="news">
  			<li></li>
  			<li></li>
  			<li></li>
  		</ul>
  	</div>
  	<div id="supporter_numbers">
  		<ul id="google_pluses">
	  		<li id="google_header" class="headlineLargeGray">
		  		<span>GOOGLE+</span>
	  		</li>
	  		<li>
		  		<div></div>
		  		<div></div>
	  		</li>
  		</ul>
  		
  		<ul id="facebook_likes">
	  		<li id="facebook_header" class="headlineLargeGray">
		  		<span>FACEBOOK</span>
	  		</li>
	  		<li>
		  		<div></div>
		  		<div></div>
	  		</li>
  		</ul>
  	</div>
  	
  	
  <ul id="supporters_footer">
	<li>
		<div class="headlineLargeGray">
			<img src="../images/buzz_sign.png"><span>SFSUPERBOWL SUPPORTERS</span>
		</div>
	</li>
	<li id="supporters_internal">
	<p>Share you excitement for a Bay Area Super Bowl with a <em>Like</em> or a <em>+1</em>.</p>
	</li>
  
	<li id="supporters_count">
	</li>
  </ul>
</div>
  
<?php include('../footer.php'); ?>